...
{
    public class BallTrajectoryCalculator : IBallTrajectoryCalculator
    {
        ...
        public BallTrajectoryCalculator(PhysicsSettings settings)
        {
        ...
        {
            // 初期角度の推定
            var delta = parameters.AimPosition - parameters.LaunchPosition;
            ...
            try
            {
                ...
                var t = horizontalDist / parameters.InitialSpeed;
                phi = Mathf.Asin((delta.y + 0.5f * g * t * t) / (parameters.InitialSpeed * t));
            }
            catch
            {
            ...
            var result = NelderMead(
                angles => SimulationError(angles, parameters),
                initialGuess
            );

            // 最適な角度から速度ベクトルを計算
            var initialVelocity = CalculateVelocityFromAngles(result.Parameters, parameters.InitialSpeed);
            var initialAngularVelocity = parameters.SpinAxis * parameters.SpinRate * 2 * Mathf.PI;

            // 軌道の計算
            var predictedPath = CalculateTrajectoryPoints(
                parameters.LaunchPosition,
                initialVelocity,
                initialAngularVelocity,
                parameters.AimPosition.z
            );

            // 結果をShotParametersに設定
            parameters.SetCalculationResults(
                initialVelocity,
                initialAngularVelocity,
                result.Value,
                predictedPath
        ...
        private float SimulationError(float[] angles, ShotParameters parameters)
        {
            var velocity = CalculateVelocityFromAngles(angles, parameters.InitialSpeed);
            var finalPos = SimulateTrajectory(
                parameters.LaunchPosition,
                velocity,
                parameters.SpinAxis * parameters.SpinRate * 2 * Mathf.PI,
                parameters.AimPosition.z
            );
            return Vector3.Distance(finalPos, parameters.AimPosition);
        }
        ...
        private Vector3[] CalculateTrajectoryPoints(Vector3 startPos, Vector3 initialVelocity, Vector3 initialSpin,
            float targetZ)
        {
            ...
            var dt = 0.016f; // 60fps相当

            points.Add(currentPos);

            while (timeElapsed < MAX_TIME && points.Count < 100)
            {
                var vMag = currentVel.magnitude;
                if (vMag < MIN_VELOCITY) break;
    ...
    public interface IBallTrajectoryCalculator
    {
